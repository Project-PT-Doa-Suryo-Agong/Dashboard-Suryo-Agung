...
import { updateContentPlanner, deleteContentPlanner } from "@/lib/services/sales.service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await updateContentPlanner(auth.ctx.supabase, id, body as Record<string, unknown>);
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return fail("DB_ERROR", "Gagal hapus content planner.", 500, error.message);
