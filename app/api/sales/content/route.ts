...
import { listContentPlanner, createContentPlanner } from "@/lib/services/sales.service";
...
export async function POST(request: Request) {
  ...
  if (!input.judul || typeof input.judul !== "string") {
    return fail("VALIDATION_ERROR", "judul wajib diisi.", 400);
  }

  const { data, error } = await createContentPlanner(auth.ctx.supabase, input);
  if (error) return fail("DB_ERROR", "Gagal membuat content plan.", 500, error.message);
  return ok({ content: data }, "Content plan berhasil dibuat.", 201);
}
