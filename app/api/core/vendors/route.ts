...
import { listVendor, createVendor } from "@/lib/services/core.service";
...
export async function POST(request: Request) {
  ...
  if (!input.nama_vendor || typeof input.nama_vendor !== "string") {
    return fail("VALIDATION_ERROR", "nama_vendor wajib diisi.", 400);
  }

  const { data, error } = await createVendor(auth.ctx.supabase, input);
