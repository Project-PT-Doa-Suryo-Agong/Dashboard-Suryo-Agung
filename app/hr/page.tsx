...
export default function HrDashboardPage() {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const employeesPayload =
          await parseJsonResponse<EmployeesListPayload>(employeesRes);
        const attendancePayload =
          await parseJsonResponse<AttendanceListPayload>(attendanceRes);
        const warningsPayload =
          await parseJsonResponse<WarningsListPayload>(warningsRes);
        ...
        setLoadError(
          error instanceof Error ? error.message : "Gagal memuat dashboard HR.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const employeeNameById = useMemo(
    () =>
      Object.fromEntries(
        employees.map((employee) => [employee.id, employee.nama]),
      ...
      attendances
        ...
        .map((item) => ({
          ...
          employeeName: item.employee_id
            ? (employeeNameById[item.employee_id] ?? "Karyawan tidak ditemukan")
  ...
  const izinCount = attendanceData.filter(
  ...
  const hadirRate =
    totalAttendance > 0 ? Math.round((hadirCount / totalAttendance) * 100) : 0;
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="rounded-lg bg-blue-500 p-2 text-blue-50">
              <Users size={18} />
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="rounded-lg bg-emerald-500 p-2 text-emerald-50">
              <UserCheck size={18} />
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:col-span-2 xl:col-span-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="rounded-lg bg-red-500 p-2 text-red-50">
                ...
                <tbody>
                  {isLoading ? (
                    <tr>
                      <td
                        colSpan={3}
                        className="px-1 py-3 text-sm text-slate-500"
                      >
                        Memuat data...
                      </td>
                    </tr>
                    ...
                    recentWarnings.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-1 py-2.5 text-sm">
                          <span
                            className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${warningBadgeClass(item.level)}`}
                          >
                            {item.level}
                          </span>
                        </td>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="space-y-3">
              {attendanceOverview.map((item) => {
                const width =
                  totalAttendance > 0
                    ? Math.round((item.value / totalAttendance) * 100)
                    : 0;
                return (
