...
import { updatePacking, deletePacking } from "@/lib/services/logistics.service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await updatePacking(auth.ctx.supabase, id, body as Record<string, unknown>);
  if (error) return fail("DB_ERROR", "Gagal update data packing.", 500, error.message);
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return fail("DB_ERROR", "Gagal hapus data packing.", 500, error.message);
