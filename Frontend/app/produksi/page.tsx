...
type QcSummary = {
  ...
  product_name: string;
  qty: number;
  status: QcInboundStatus | QcOutboundStatus;
  date: string;
};
...
const outbound_result_label: Record<"passed" | "failed" | "rework", string> = {
  passed: "Passed",
  failed: "Failed",
  rework: "Rework",
};

const outbound_result_class: Record<"passed" | "failed" | "rework", string> = {
...
export default function ProduksiDashboardPage() {
  ...
  const antrean_qc_inbound_count = useMemo(() => {
    return production_t_qc_inbound_rows.filter((item) => item.status === "pending").length;
  }, [production_t_qc_inbound_rows]);

  const outbound_checked_rows = useMemo(() => {
    return production_t_qc_outbound_rows.filter((item) => item.status !== "pending");
  }, [production_t_qc_outbound_rows]);

  const outbound_passed_rate = useMemo(() => {
    if (outbound_checked_rows.length === 0) return 0;
    const passed_count = outbound_checked_rows.filter((item) => item.status === "passed").length;
        ...
        <article className="relative overflow-hidden rounded-xl border border-amber-200 bg-white p-4 md:p-6 shadow-sm">
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div className="min-w-0 space-y-1">
              ...
              <p className="text-xs md:text-sm text-amber-700/80">Bahan baku menunggu pengecekan</p>
            </div>
            ...
            <table className="w-full table-fixed">
              ...
              <tbody className="divide-y divide-slate-100">
                {hasil_qc_terbaru_rows.map((item) => {
                  const resultStatus = item.status as "passed" | "failed" | "rework";
                  return (
                    <tr key={item.id}>
                      <td className="py-3 pr-2 text-sm font-semibold text-slate-800 break-all">{item.reference_id}</td>
                      <td className="py-3 pr-2 text-sm text-slate-700 break-words">{item.product_name}</td>
                      <td className="py-3">
                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold whitespace-nowrap ${outbound_result_class[resultStatus]}`}>
                          {outbound_result_label[resultStatus]}
                        </span>
                      </td>
                    </tr>
                  );
